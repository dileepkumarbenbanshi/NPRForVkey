...
extension DBManagerMemberDetail {
    ...
    func updateMemberStatusSkipedHH(hhModel:NPR_2021hh_Details)  {
        ...
        if hhModel.hh_status ?? "" == HHStatusCode.locked  || hhModel.hh_status ?? "" == HHStatusCode.migratedOut  || hhModel.hh_status ?? "" == HHStatusCode.locked {
        ...
        fetchedHHMembers(modelSelectedHH: hhModel) { (totalMemberModel) in
            ...
            for memberModel in totalMemberModel {
                ...
                if memberLivingStatus == .migratedOut || memberLivingStatus == .notAvailable || memberLivingStatus == .lockedMember{
