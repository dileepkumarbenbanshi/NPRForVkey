...
class DBManager_CountryStateDistrict {
    ...
    func getSubDistrictCode(stateCode:String,tahsilName:String) -> String {
        if stateCode.isEmpty || tahsilName.isEmpty  {
            return ""
        }
        
        let predicate = NSPredicate(format: "name = %@ AND state_code = %@ ",tahsilName,stateCode )
    ...
    func getSubDistrictName(stateCode:String,districtCode:String,tahsilCode:String) -> String {
        if stateCode.isEmpty || tahsilCode.isEmpty  {
            return ""
        }
        let trimedDistrictCode = districtCode.trimmingCharacters(in: .whitespaces)
        let trimedTahsilCode = tahsilCode.trimWhiteSpace()
        let trimmedStateCode = stateCode.trimWhiteSpace()
        
        
        let predicate = NSPredicate(format: "id = %@ AND district_code = %@ AND state_code = %@ ",trimedTahsilCode,trimedDistrictCode,trimmedStateCode )
    ...
    func getDistrictName(stateCode:String,districtCode:String) -> String {
        ...
        let trimedDistrictCode = districtCode.trimmingCharacters(in: .whitespaces)
        let predicate = NSPredicate(format: "id = %@ AND stateCode = %@ ",trimedDistrictCode,stateCode)
