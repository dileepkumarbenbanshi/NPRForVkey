...
class NPRURLRequestSession: NSObject {
    ...
    public func getOtpChangePassword( params:[String: String],finished: @escaping (Bool,Any,CustomServiceError?) -> Void) {
       ...
       public func postUploadDataRequest(params:[String: Any],finished: @escaping (Bool,Any,CustomServiceError?) -> Void) {
        ...
        if !util.isEnumerator() {
            endPoint = EndPoint.Super_UploadData
        }
    ...
    public func callAPI_download2019Data( params:[String: Any],finished: @escaping (Bool,Any,CustomServiceError?) -> Void) {
        self.request(BaseUrl.url + EndPoint.npr_2019Data_download , httpMethod: RequestType.POST.rawValue, apiType: APIType.NonDeviceManagement, parameter: params) { (success, result, error) in
            finished(success,result, error)
        }
    }
